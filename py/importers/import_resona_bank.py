...
COL_TEKIYO = 19       # 摘要
COL_ACCOUNT_NO = 11   # 口座番号(自然キーの口座部分。りそなは行ごとに口座を持つ)
MIN_COLS = 21
    ...
    with open(csv_path, "r", encoding=ENCODING) as f:
        ...
        for row in reader:
            ...
            transactions.append({
                ...
                "account_no": row[COL_ACCOUNT_NO].strip(),
            })
    return transactions
...
def import_resona_bank_csv(csv_path, db_path: str = None):
    """りそな銀行CSVを複式簿記スキーマでDBに登録する。

    記帳は py/importers/ledger.py(記帳サービス)に委ね、この関数は
    「CSV → Posting の列」だけを担う。冪等・鍵の生成・墓標の参照はサービス側。
    """
    ...
    try:
        ...
        for t in transactions:
            amount = t["amount"]
            signed = amount if t["txn_name"] == "入金" else -amount
            if t["txn_name"] == "入金":
            ...
            posting = ledger.Posting(
                source="resona_bank",
                account_key=t.get("account_no") or "main",
                date=t["date"],
                description=t["description"],
                amount=signed,
                balance=_to_int(t["balance"]) if t["balance"] else None,
                seq=seq.next(t["date"], t["description"], signed),
                entries=entries,
                legacy_fitids=(_make_fitid(t),),   # v1 で既に入っている行は再記帳しない
            )
            try:
